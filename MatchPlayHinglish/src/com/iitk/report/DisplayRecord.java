...
public class DisplayRecord  extends Activity
{
        ...
        public void displayRecords() 
        {
    		...
    		mDbHelper.close();
    		System.out.println("Total Records=="+student.size());
    	}
        ...
        public void createCSV(ArrayList<Student> studentdata,String name)
        {
        	...
        	String[] headers = new String[]{"Name","Date","Match alphabets and numbers","Total Mistakes"};
        	try 
            {
            String path = Environment.getExternalStorageDirectory().getAbsolutePath() + "/AppsReport/MatchPlayHinglish";
            ...
            for(int i=0;i<studentDataCsv.size();i++)
            {
                ...
                mistake.append(st.getTotalMistakes());
                mistake.append("\n");//adding new line after record
                
                writeCsvData(st.getName(), st.getDate(),level1.toString(),mistake.toString());
            }
            writer.flush();
            writer.close(); 
            }catch(IOException e) 
        ...
        private void writeCsvData(String name,String date,String level1,String totalMistake) throws IOException 
        {  
        	  String line = String.format("%s,%s,%s,%s",name.toUpperCase(),date,level1,totalMistake);
        	  writer.write(line);
        }
        ...
        public void sdCardPath(String name)
        {
        	...
        	sb.append("File Path : sdcard/AppsReport/MatchPlayHinglish \n");
